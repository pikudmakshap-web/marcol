...
function Users() {
    ...
    const handleDelete = async (id) => {
        ...
        if (window.confirm("האם אתה בטוח שברצונך למחוק משתמש זה?")) {
            try {
                await deleteUser(id);
                // The socket payload 'user_deleted' updates the store directly
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            dataToSave.fullName = (dataToSave.fullName || '').trim();
            ...
            // The socket payload 'user_added' or '_updated' updates the store directly
    ...
    const filteredUsers = users.filter(u => {
        ...
        if (filterType === 'active') matchesFilter = u.isOnline;
    ...
    const getRoleBadge = (role) => {
        switch (role) {
            ...
            case 'officer': return <span className="px-3 py-1 bg-orange-50 text-orange-600 border border-orange-100 rounded-full text-xs font-bold">רס"פ / קצין ניהול </span>;
    ...
    return (
        <div className="space-y-6 pb-20" data-tour="users-page">
            {/* Page Sub-Header Actions */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6" data-tour="users-header">
                <div>
                    <h1 className="text-3xl font-bold text-slate-800">משתמשים והרשאות</h1>
                    <p className="text-slate-400 text-sm mt-1">ניהול חשבונות משתמש והגדרות גישה</p>
                </div>
                <button 
                    onClick={handleOpenAdd} 
                    className="bg-[#526f52] hover:bg-[#435c43] text-white px-6 py-3 rounded-full shadow-lg shadow-[#526f52]/20 transition-all flex items-center gap-2 font-bold"
            ...
            {loading ? <LoadingSpinner fullScreen={false} /> : (
                <>
                    {/* Main Content Card */}
                    <div className="bg-white rounded-[32px] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 overflow-hidden" data-tour="users-main-card">
                        {/* Toolbar */}
                        <div className="p-6 border-b border-gray-50 flex flex-col sm:flex-row items-center justify-between gap-4" data-tour="users-toolbar">
                            ...
                            <div className="flex gap-2 w-full sm:w-auto overflow-x-auto no-scrollbar">
                                ...
                                >
                                    פעילים ({users.filter(u => u.isOnline).length})
                                </button>
                                <button
                        ...
                        {/* Table */}
                        <div className="overflow-x-auto" data-tour="users-table" dir="rtl">
                            <table className="w-full text-right">
                                <thead className="bg-[#f8f9f8] text-gray-500 text-xs uppercase font-bold tracking-wider">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-right">נוכחות</th>
                                        <th className="px-6 py-4 text-right">סטטוס</th>
                                        <th className="px-6 py-4 text-right rounded-tl-[32px]">פעולות</th>
                                    </tr>
                                </thead>
                                {filteredUsers.map((user) => (
                                    <tr key={user.id} className="hover:bg-[#fcfdfc] transition-colors group">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {user.isOnline ? (
                                                ...
                                                <div className="flex flex-col text-xs text-gray-500">
                                                    <span className="font-medium text-gray-400">לא מחובר</span>
                                                    <span className="text-[10px] text-gray-400 mt-0.5">
                                                        נראה לאחרונה: {user.lastSeen ? formatDateTime(user.lastSeen) : 'לא ידוע'}
                                                    </span>
                                                </div>
                                            )}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {!user.isActive ? (
                                                <span className="text-xs font-bold text-gray-400">חשבון חסום</span>
                                            ) : (
                                                <span className={`text-xs font-bold ${user.isOnline ? 'text-green-600' : 'text-red-400'}`}>
                                                    {user.isOnline ? 'פעיל' : 'לא פעיל'}
                                                </span>
                                            )}
                                        </td>

...
function Products() {
    ...
    const { products, loading, loadingMore, error, hasMore, total, fetchProducts, loadMoreProducts } = useProductStore();
    ...
    const canUpdateStock = isAdmin || isCashier;

    const isCorruptedText = (value) => {
        if (!value || typeof value !== 'string') return false;
        return /�|Ã|ן¿½|����/.test(value);
    };
    ...
    const hasCategorySelection = systemMode === 'category' && walletCategories.length > 0;
    ...
    useEffect(() => {
        fetchProducts({ force: true, reset: true, limit: 100 });
        fetchCategories();
        fetchWallets({ force: true, reset: true, limit: 50 });

        const settingsCacheKey = 'settings-cache-v1';
        ...
        if (cachedSettings) {
            ...
            getSettings().then(data => {
                if (data) {
                    ...
                    sessionStorage.setItem(settingsCacheKey, JSON.stringify(data));
                }
    ...
    }, [fetchProducts, fetchCategories, fetchWallets, user?.environmentId]);
    ...
    const handleOpenEdit = async (product) => {
        ...
        if (product?.id && product.imageUrl === undefined) {
            try {
                productForEdit = await getProduct(product.id);
            } catch {
        ...
        setFormData({
            ...
            category: productForEdit.category === "ללא קטגוריה" ? apiCategories[0]?.name : productForEdit.category || '',
            unitPrice: productForEdit.unitPrice,
            supplierName: productForEdit.supplierName || '',
            imageUrl: productForEdit.imageUrl || '',
            initialStock: productForEdit.quantity || 0,
            isActive: productForEdit.isActive,
        });
        setIsModalOpen(true);
    };
    ...
    const handleImageChange = async (e) => {
        ...
        if (file) {
            try {
                ...
                toast.success('התמונה הועלתה מקומית בהצלחה');
            } catch (error) {
    ...
    const handleAdjustStock = (productId, delta) => {
        ...
        if (isCashier && newQty < currentQty) {
            toast.error("אין הרשאה להוריד כמות במלאי");
            return;
        }
        ...
        try {
            ...
            if (isEditMode) {
                if (!currentProduct?.id) {
                    toast.error('לא נמצא מזהה מוצר לעריכה. רענן עמוד ונסה שוב.');
                    return;
                }
    ...
    return (
        <div className="space-y-6 pb-20 relative" data-tour="products-page">
            {isCashier && (
                <div className="absolute left-0 top-0 z-30 flex items-center gap-2">
                    <button
                        type="button"
            ...
            {/* Page Header Actions */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2" data-tour="products-header">
                <div>
                    <div className="flex items-baseline gap-2">
                        <h1 className="text-3xl font-bold text-slate-800">מוצרים</h1>
                        <span className="text-sm font-medium text-slate-400 bg-slate-100 px-2 py-0.5 rounded-lg">
                            סה"כ: {total || products.length} מוצרים
                            {total > products.length ? ` | נטענו ${products.length}` : ''}
                        </span>
                    </div>
                    <p className="text-slate-400 text-sm mt-1">נהל את מלאי המוצרים בחנות</p>
                </div>
            </div>
            {/* Filters Bar */}
            <div className="bg-white rounded-[24px] p-2 shadow-sm border border-gray-100 flex flex-col md:flex-row items-center gap-2 relative z-20" data-tour="products-filters">
                {/* Search - Global */}
            ...
            {/* Products Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" data-tour="products-grid">
                    ...
                    >
                        ...
                        <div className="h-32 mb-4 mt-2 relative flex items-center justify-center shrink-0 rounded-2xl overflow-hidden">
                            <ProductImage
                                ...
                                imageClassName="relative z-10 max-w-[95%] max-h-[95%] object-contain mix-blend-multiply transition-transform duration-500 group-hover:scale-125"
                        ...
                        <div className="space-y-2 px-1 flex-1 flex flex-col justify-between">
                            <div>
                                <h3 className="font-bold text-[#2d3748] text-lg leading-tight truncate" title={safeText(product.name, 'מוצר')}>
                                    {safeText(product.name, 'מוצר')}
                                </h3>
                                <div className="flex justify-between items-center mt-1">
                                    <p className="text-gray-400 text-xs">{safeText(product.category, 'ללא קטגוריה')}</p>
                                ...
                                {safeText(product.description) ? (
                                    <p className="text-gray-300 text-xs mt-1.5" title={safeText(product.description)}>
                                        {safeText(product.description).split(' ').length > 10 
                                            ? safeText(product.description).split(' ').slice(0, 10).join(' ') + '...' 
                                            : safeText(product.description)}
                                    </p>
                                ) : (
                                        ...
                                        <>
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (isCashier && newQty < (product.quantity || 0)) {
                                                        toast.error("אין הרשאה להוריד כמות במלאי");
                                                        return;
                                                    }
                                                    setPendingStock(prev => ({ ...prev, [product.id]: newQty }));
                                                }}
            ...
            {hasMore && (
                <div className="flex justify-center">
                    ...
                    >
                        {loadingMore ? 'טוען עוד...' : 'טען עוד מוצרים'}
                    </button>
                </div>
            )}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        ...
                                        <div className="flex flex-col gap-2 md:col-span-1">
                                            ...
                                            <div className="text-center">
                                                ...
                                                {isUploadingImage && (
                                                    <p className="text-[11px] text-[#526f52] mt-1">מעלה ל-SharePoint...</p>
                                                )}
                                            </div>
                                        </div>

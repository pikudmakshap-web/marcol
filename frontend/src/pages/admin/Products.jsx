...
function Products() {
    ...
    const [formData, setFormData] = useState({
        ...
        unitPrice: '', supplierName: '', imageUrl: '', initialStock: '', isActive: true
    });
    ...
    useEffect(() => {
        ...
        usedCategories.forEach(cat => {
            const normalizedId = cat.toLowerCase(); // Simple ID generation
            if (!newCategories.some(c => c.id === normalizedId || c.label === cat)) {
                const existing = newCategories.find(c => c.id === cat || c.label === cat);
                if (!existing) {
                    newCategories.push({ id: cat, label: cat });
                }
            }
        });
    ...
    const handleSaveStock = async (product) => {
        ...
        useProductStore.getState().updateProductState(product.id, { inventory: { ...product.inventory, quantity: newQuantity } });
        ...
        try {
            ...
            fetchProducts(true); // Optional: ensure absolute sync with server
        } catch (err) {
            console.error("Failed to update stock", err);
            // Revert changes (reload data)
            fetchProducts(true);
        }
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            // Ensure category is set (default to general if empty)
            ...
            // Update categories list immediately if new category used
            if (dataToSave.category) {
                const cat = dataToSave.category;
                if (!allCategories.some(c => c.label === cat || c.id === cat)) {
                    setAllCategories([...allCategories, { id: cat, label: cat }]);
                }
            }
        } catch (err) {
    ...
    const getStatusColor = (quantity, isActive) => {
        if (!isActive) return 'bg-gray-100 text-gray-500 border-gray-200';
        const qty = quantity || 0;
        if (qty === 0) return 'bg-[#ffebee] text-[#c62828] border-[#ffcdd2]';
        if (qty < systemSettings.lowStockThreshold) return 'bg-[#fff3e0] text-[#ef6c00] border-[#ffe0b2]';
        return 'bg-[#e8f5e9] text-[#2e7d32] border-[#c8e6c9]';
    };

    const getStatusText = (quantity, isActive) => {
        if (!isActive) return 'לא פעיל';
        const qty = quantity || 0;
        if (qty === 0) return 'אזל';
        if (qty < systemSettings.lowStockThreshold) return 'נמוך';
        return 'במלאי';
    };
                ...
                <div className="flex items-center gap-2 w-full md:w-auto px-2">
                    ...
                    {moreCategories.length > 0 && (
                        <div className="relative" ref={categoryMenuRef}>
                            ...
                            {/* Dropdown */}
                            {showCategoryMenu && (
                                <div className="absolute top-12 left-0 min-w-[160px] bg-white rounded-xl shadow-xl border border-gray-100 p-2 flex flex-col gap-1 z-30 max-h-60 overflow-y-auto custom-scrollbar">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredProducts.map((product) => (
                    ...
                    >
                        ...
                        <div className="h-40 rounded-[24px] overflow-hidden mb-3 relative bg-gray-50 shrink-0">
                            <img
                            ...
                            <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 backdrop-blur-[2px]">
                                <button
                                    ...
                                    className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-gray-600 hover:text-[#526f52] shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-all duration-300"
                                >
                                    <span className="material-symbols-outlined text-[20px]">edit</span>
                                </button>
                                <button
                                    onClick={() => handleDelete(product.id)}
                                    className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-gray-600 hover:text-red-500 shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 delay-75"
                                >
                            ...
                            {/* Price Tag */}
                            <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-full shadow-sm text-sm font-bold text-[#2d3748]">
                            ...
                            <div>
                                ...
                                <div className="flex justify-between items-center mt-1">
                                    ...
                                    <p className="text-xs text-gray-300">
                                        {product.sku ? `מקט: ${product.sku}` : 'אין מידע על מקט'}
                                    </p>
                                </div>
                            </div>

                            <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                                <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(product.inventory?.quantity, product.isActive)}`}>
                                    {getStatusText(product.inventory?.quantity, product.isActive)}
                                </span>
                                    ...
                                    {pendingStock[product.id] !== undefined ? (
                                        ...
                                        <>
                                            <span className="text-sm font-bold text-[#2d3748] px-2 whitespace-nowrap">
                                                מלאי: {product.inventory?.quantity || 0}
                                            </span>
                                            <button
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    setPendingStock(prev => ({ ...prev, [product.id]: product.inventory?.quantity || 0 }));
                                                }}
                                            ...
                                            <div>
                                                ...
                                                <div className="relative">
                                                    <input
                                                        type="text"
                                                        list="category-list"
                                                        className="w-full px-4 py-3 rounded-xl bg-gray-50 border-none text-sm focus:ring-2 focus:ring-[#526f52]/20 text-[#2d3748]"
                                                ...
                                                <p className="text-[10px] text-gray-400 mt-1 text-center">בחר או הקלד קטגוריה חדשה</p>
                                            </div>
                                        </div>
                                    </div>

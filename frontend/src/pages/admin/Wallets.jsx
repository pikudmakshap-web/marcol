...
function Wallets() {
    ...
    const { wallets, loading: walletsLoading, loadingMore: walletsLoadingMore, error: walletsError, hasMore: walletsHasMore, total: walletsTotal, fetchWallets, loadMoreWallets } = useWalletStore();
    ...
    const loading = walletsLoading || ((currentUser?.role === 'admin' || currentUser?.role === 'superadmin') && usersLoading);
    const error = walletsError || ((currentUser?.role === 'admin' || currentUser?.role === 'superadmin') && usersError);
    ...
    const categoryColorMap = useMemo(() => {
        ...
        apiCategories.forEach((cat) => {
            ...
            const baseColor = isColorMissing(cat?.color)
                ? getFallbackCategoryColor(cat.name)
                : cat.color;

            const color = toBackgroundRgba(baseColor, 0.15);
    ...
    const getCategoryBackgroundColor = (categoryName) => {
        ...
        return categoryColorMap.get(key) || toBackgroundRgba(getFallbackCategoryColor(categoryName), 0.15);
    };

    const getCategorySolidColor = (categoryName) => {
        const bgColor = getCategoryBackgroundColor(categoryName);
        return toStrongSolidColor(bgColor);
    };
    ...
    }, [fetchWallets, fetchCategories, fetchUsers, currentUser?.role, currentUser?.environmentId]);
    ...
    const handleDelete = async (e, id) => {
        ...
        if (window.confirm(confirmMsg)) {
            try {
                ...
                fetchWallets({ force: true, reset: true, limit: 50 }); // Refresh global state
                if (isLastWallet) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (systemMode === 'general' && formData.categories.length > 0) {
            toast.error('במצב תקציב כללי אי אפשר לשמור ארנק עם קטגוריות');
            return;
        }
        try {
            const dataToSave = {
            ...
            fetchWallets({ force: true, reset: true, limit: 50 }); // Refresh global list
    ...
    const filteredWallets = wallets.filter((w) => {
        ...
        return (w.name?.toLowerCase().includes(search) || w.walletNumber?.includes(search));
    });
    ...
    const handleExportToExcel = (e) => {
        ...
        const exportData = filteredTransactions.map(tx => {
            ...
            return {
                ...
                'מבצע': (tx.officer?.role === 'admin' || tx.officer?.role === 'superadmin') ? 'מנהל מערכת' : (tx.officer?.fullName || ((tx.cashier?.role === 'admin' || tx.cashier?.role === 'superadmin') ? 'מנהל מערכת' : tx.cashier?.fullName) || 'מערכת'),
    ...
    return (
        <div className="space-y-8 pb-20 relative" data-tour="wallets-page">
            ...
            {currentUser?.role === 'cashier' && (
                <div className="absolute left-2 top-2 z-30 flex items-center gap-2">
                    <button
                        ...
                        className="w-11 h-11 rounded-2xl bg-white border border-slate-200 text-[#526f52] shadow-sm hover:shadow-md hover:bg-slate-50 transition-all flex items-center justify-center"
                        title="מעבר למוצרים"
                    >
            ...
            {/* Page Header Actions */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6" data-tour="wallets-header">
                <div>
                    <div className="flex items-baseline gap-2">
                        <h1 className="text-3xl font-bold text-slate-800">ניהול תקציבים</h1>
                        <span className="text-sm font-medium text-slate-400 bg-slate-100 px-2 py-0.5 rounded-lg">
                            סה"כ: {walletsTotal || wallets.length} ארנקים
                            {walletsTotal > wallets.length ? ` | נטענו ${wallets.length}` : ''}
                        </span>
                    </div>
                    <p className="text-slate-400 text-sm mt-1">צפה ונהל את ארנקי היחידה</p>
                </div>
                
                <div className="flex flex-col sm:flex-row items-center gap-3">
                    <div className="relative w-full sm:w-64" data-tour="wallets-search">
                        <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">search</span>
                        <input
                            type="text"
                            placeholder="חפש ארנק..."
                            value={walletSearchTerm}
                            onChange={(e) => setWalletSearchTerm(e.target.value)}
                            className="w-full bg-white border border-slate-200 rounded-full py-2.5 pr-10 pl-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#526f52]/20 focus:border-[#526f52] transition-all shadow-sm"
                        />
                    </div>
                    ...
                    {currentUser?.role === 'officer' && (
                        <button
                            ...
                            className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center shadow-sm hover:shadow-md transition-all border border-slate-100 text-[#526f52]"
                            title="צפייה בהודעת מערכת"
                        >
                    ...
                    {(currentUser?.role === 'admin' || currentUser?.role === 'superadmin') && (
                        <button
                            ...
                            className="bg-[#526f52] hover:bg-[#435c43] text-white px-6 py-2.5 rounded-full shadow-lg shadow-[#526f52]/20 transition-all flex items-center gap-2 font-bold whitespace-nowrap"
                            data-tour="wallets-add-button"
                        >
            ...
            {/* Main Content Area */}
            <div className={`flex flex-col lg:flex-row gap-6 transition-all duration-500 items-start`} data-tour="wallets-main-area">
                {/* Right Side (List of Wallets) */}
                <div className={`${selectedWalletId ? 'lg:w-1/3' : 'w-full'} transition-all duration-500`}>
                    <div className={`grid ${selectedWalletId ? 'grid-cols-1 gap-4' : 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6'} transition-all duration-500`} data-tour="wallets-grid">
                        {filteredWallets.map((wallet, index) => {
                            ...
                            return (
                                ...
                                >
                                            ...
                                            return (
                                                <div className="relative mt-2">
                                                    <div className="flex flex-row gap-1.5 flex-wrap">
                                                        {visible.map(cat => {
                                                            const catColor = getCategoryBackgroundColor(cat.categoryName);
                                                            const solidColor = getCategorySolidColor(cat.categoryName);
                                                            const isWhite = isWhiteLikeColor(catColor);
                                                            return (
                                                                <div
                                                                    key={cat.categoryName}
                                                                    className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold whitespace-nowrap"
                                                                    style={{
                                                                        backgroundColor: isWhite ? '#f3f4f6' : catColor,
                                                                        color: isWhite ? '#555' : solidColor,
                                                                        border: `1px solid ${isWhite ? '#e5e7eb' : toBackgroundRgba(solidColor, 0.3)}`
                                                                    }}
                                                                >
                                                                    <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: isWhite ? '#aaa' : toBackgroundRgba(solidColor, 0.7) }}></span>
                                    ...
                                    {/* Footer Indicators */ }
                                            <div className="-mx-6 -mb-6 mt-4 px-6 py-4 border-t border-gray-100 bg-[#f8fafc]">
                    ...
                    {filteredWallets.length === 0 && (
                        <div className="mt-4 bg-white rounded-2xl border border-gray-100 p-6 text-center text-gray-500">
                            לא נמצאו ארנקים להצגה
                        </div>
                    )}
                    {walletsHasMore && (
                        <div className="mt-4 flex justify-center">
                            <button
                                ...
                                disabled={walletsLoadingMore}
                                className="px-6 py-3 rounded-full bg-white border border-slate-200 text-[#526f52] font-bold hover:bg-slate-50 disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
                            >
                                {walletsLoadingMore ? 'טוען עוד...' : 'טען עוד ארנקים'}
                            </button>
                        </div>
                    )}
                </div>
                    ...
                    {selectedWalletId && (
                        <div className="lg:w-2/3 w-full bg-white rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.08)] flex flex-col overflow-hidden animate-fade-in-up border border-[#526f52]/10 sticky top-6 max-h-[calc(100vh-100px)]" data-tour="wallets-details-panel">
                            {/* Header Details */}
                                                        ...
                                                        <div className="absolute bottom-full mb-1 left-0 bg-white rounded-2xl shadow-xl border border-gray-100 p-3 z-50 min-w-[200px] max-w-[300px]">
                                                            ...
                                                            <div className="flex flex-col gap-1.5">
                                                                {overflow.map(cat => {
                                                                    ...
                                                                    const isWhite = isWhiteLikeColor(catColor);
                                                                    return (
                                                                        <div key={cat.categoryName} className="flex items-center justify-between gap-2 px-2 py-1 rounded-lg" style={{ backgroundColor: isWhite ? '#f9fafb' : catColor }}>
                                                                            <span className="text-[11px] font-bold" style={{ color: isWhite ? '#555' : solidColor }}>{cat.categoryName}</span>
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {walletTransactions
                                                .filter(tx => {
                                                    ...
                                                    if (!searchTerm) return true;
                                                    const dateStr = formatDateTime(tx.createdAt).toLowerCase();
                                                    const userName = ((tx.officer?.role === 'admin' || tx.officer?.role === 'superadmin') ? 'מנהל מערכת' : (tx.officer?.fullName || ((tx.cashier?.role === 'admin' || tx.cashier?.role === 'superadmin') ? 'מנהל מערכת' : tx.cashier?.fullName) || 'מערכת')).toLowerCase();
                                                    const typeStr = (tx.transactionType === 'sale' ? 'קנייה' : 'זיכוי/החזרה').toLowerCase();
                                                    const amountStr = tx.totalAmount.toString().toLowerCase();
                                                    return dateStr.includes(searchTerm) || userName.includes(searchTerm) || typeStr.includes(searchTerm) || amountStr.includes(searchTerm);
                                                })
                                                    ...
                                                    <React.Fragment key={tx.id}>
                                                        ...
                                                        >
                                                            ...
                                                            <td className="px-6 py-4 text-sm font-bold text-[#2d3748] whitespace-nowrap">
                                                                {(tx.officer?.role === 'admin' || tx.officer?.role === 'superadmin') ? 'מנהל מערכת' : (tx.officer?.fullName || ((tx.cashier?.role === 'admin' || tx.cashier?.role === 'superadmin') ? 'מנהל מערכת' : tx.cashier?.fullName) || 'מערכת')}
                                                            </td>
                ...
                {isModalOpen && (
                    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                        ...
                        <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-2xl relative z-10 overflow-hidden flex flex-col max-h-[90vh]" data-tour="wallet-create-modal">
                            ...
                            <div className="p-8 overflow-y-auto custom-scrollbar">
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    <div className="mt-8">
                                        {wallets.length > 0 && (
                                            <div className="mb-4 bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center gap-2 text-amber-700" data-tour="wallet-mode-lock-warning">
                                        ...
                                        <div className={`p-1.5 bg-gray-100/80 rounded-[14px] flex items-center justify-between gap-1.5 mb-6 shadow-inner transition-all duration-300 ${tabWarning ? 'ring-2 ring-red-400 bg-red-50/50 scale-[1.02]' : ''}`} data-tour="wallet-budget-mode-toggle">
                                            <button
                                                type="button"
                                        ...
                                        <div className="bg-white border border-gray-100 rounded-3xl shadow-sm p-6 overflow-hidden relative">
                                            {activeModalTab === 'general' ? (
                                                <div className="animate-fade-in-up" data-tour="wallet-general-budget-input">
                                                ...
                                                <div className="animate-fade-in-up space-y-4" data-tour="wallet-categories-panel">
                                                        ...
                                                        {formData.categories.length > 0 && (
                                                            <button
                                                                ...
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    if (systemMode === 'categories') {
                                                                        toast.error('המערכת נעולה על מצב קטגוריות. אי אפשר לנקות את כל הקטגוריות.');
                                                                        return;
                                                                    }
                                                                    setFormData(prev => ({
                                                                        ...prev,
                                                    ...
                                                    <div className="flex flex-col sm:flex-row gap-3" data-tour="wallet-categories-input">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-gray-500">
                                                            {user.isOnline ? (
                                                                <span className="text-green-600 font-medium text-[10px]">מחובר כעת</span>
                                                            ) : user.lastSeen ? (
                                                                <span className="text-[10px] text-gray-400">נראה לאחרונה: {formatDateTime(user.lastSeen)}</span>
                                                            ) : (
                                                                <span className="text-[10px] text-gray-400">המשתמש לא התחבר למערכת</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                </div>

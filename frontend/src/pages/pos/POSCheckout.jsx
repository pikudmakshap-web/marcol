...
const POSCheckout = () => {
    ...
    const handlePaymentSearch = async (e) => {
        ...
        try {
            ...
            if (results.length === 0) {
                setPaymentError('לא נמצא ארנק תואם');
        ...
        setOrderItems(prevItems => {
            return prevItems.map(item => {
                if (item.id === id) {
                    ...
                    const productStock = item.inventory?.quantity || 0;
                    ...
                    {loading ? (
                        ...
                        <>
                            ...
                            <div className="text-center mb-8 shrink-0 flex flex-col justify-center items-center relative z-10 w-full mt-2">
                                ...
                                <div className="flex justify-center gap-4 mb-8 w-full max-w-sm">
                                    <button onClick={() => setIsManualBarcodeOpen(true)} className="flex-1 bg-[#ecfdf5] hover:bg-[#d1fae5] text-[#3ce619] py-4 rounded-full font-bold text-lg transition-transform hover:scale-105 border-2 border-[#3ce619]/20 shadow-sm flex items-center justify-center gap-2">
                                        <span className="material-symbols-outlined">keyboard</span>
                                        הקלדת ברקוד
                                    </button>
                                    <button onClick={() => setIsPriceCheckOpen(true)} className="flex-1 bg-[#ecfdf5] hover:bg-[#d1fae5] text-[#3ce619] py-4 rounded-full font-bold text-lg transition-transform hover:scale-105 border-2 border-[#3ce619]/20 shadow-sm flex items-center justify-center gap-2">
                                ...
                                <div className="flex flex-col h-full overflow-hidden">
                                    ...
                                    <div className="flex flex-col gap-3 pb-4 pr-1">
                                        {categoryProducts.map(product => {
                                            ...
                                            const safePrice = product?.unitPrice ? Number(product.unitPrice) : 0;
                                            return (
                                                <button key={product.id} onClick={() => handleAddToCart(product)} className="bg-white hover:bg-gray-50 border border-white/50 rounded-2xl p-4 flex items-center justify-between transition-transform hover:scale-[1.02] shadow-sm text-[#166534]  text-right w-full">
                                                    {/* Product Details (Right) */}
                                                    <div className="flex items-center gap-3 overflow-hidden flex-1 pl-2">
                                                        <div className="w-10 h-14 bg-gray-50 rounded-xl overflow-hidden shrink-0 border border-gray-100 flex items-center justify-center">
                                                            {product.imageUrl ? (
                                                        ...
                                                        <div className="flex flex-col overflow-hidden">
                                                            <span className="font-bold text-sm sm:text-base leading-tight truncate px-1">{product.name}</span>
                                                    ...
                                                    <div className="shrink-0 text-left">
                                                        <span className="bg-[#ecfdf5] text-[#166534] px-3 py-1 rounded-full font-black text-lg border border-[#3ce619]/20">₪{safePrice.toFixed(2)}</span>
                                                    </div>
                                                </button>
                                            );
                                        })}
                                        ...
                                        {manualBarcodeResults.length > 0 ? (
                                            manualBarcodeResults.map(p => (
                                                <button
                                                    ...
                                                    className="flex justify-between items-center bg-[#ecfdf5] hover:bg-[#d1fae5] border border-[#3ce619]/20 hover:border-[#3ce619]/50 p-4 rounded-xl transition-colors text-right w-full"
                                                >
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-10 h-10 bg-white rounded-lg border border-gray-100 flex items-center justify-center overflow-hidden shrink-0 shadow-sm">
                                                            {p.imageUrl ? (
                                                        ...
                                                        <div className="flex flex-col">
                                                            <span className="font-bold text-[#2d3748]">{p.name}</span>
                                                            <span className="text-xs text-gray-500">{p.barcode || 'ללא ברקוד'}</span>
                                                        </div>
                                                    </div>
                        ...
                        <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-lg relative z-10 overflow-hidden flex flex-col">
                            ...
                            <div className="p-8 flex flex-col items-center w-full">
                                ...
                                <form onSubmit={handlePaymentSearch} className="w-full relative mb-4">
                                    <input
                                        type="text"
                                        placeholder="הקלד כאן או סרוק ברקוד ולחץ אנטר..."
                                        autoFocus
                                        value={paymentQuery}
                    ...
                    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
                        ...
                        <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-md relative z-10 overflow-hidden flex flex-col">
                            ...
                            <div className="p-8 flex flex-col items-center w-full">
                                ...
                                <input
                                    type="text"
                                    placeholder="דוגמא: חלב, או 729000... 🔍"
                                    autoFocus
                                            ...
                                            priceCheckResults.map(p => (
                                                <div key={p.id} className="flex justify-between items-center bg-gray-50 border border-gray-100 p-4 rounded-xl">
                                                    <div className="flex items-center gap-3 flex-1">
                                                        <div className="w-10 h-10 bg-white rounded-lg border border-gray-200 flex items-center justify-center overflow-hidden shrink-0 shadow-sm">
                                                            {p.imageUrl ? (
                                                                <img src={p.imageUrl} alt={p.name} className="w-full h-full object-cover" />
                                                            ) : (
                                                                <span className="material-symbols-outlined text-gray-300 text-xl">image</span>
                                                        ...
                                                        <span className="font-bold text-sm text-[#2d3748] text-right">{p.name}</span>
                                                    </div>
                                                    <span className="text-[#3ce619] font-black text-xl">₪{(p.unitPrice || 0).toFixed(2)}</span>
                                                </div>
                                            ))
                                        ) : (
                                            <div className="text-center text-gray-400 py-4 text-sm font-bold">לא נמצא מוצר תואם.</div>

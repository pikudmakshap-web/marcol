...
function Products() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            // Ensure category is set (default to כללי if empty)
            const dataToSave = { ...formData };
            if (!dataToSave.category) dataToSave.category = 'כללי';
        ...
        <div className="space-y-6 pb-20 relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredProducts.map((product) => (
                    ...
                    >
                        {/* Smaller Image Container (No Background Box) */}
                        <div className="h-32 mb-4 mt-2 relative flex items-center justify-center shrink-0">
                            {/* Blurred Background */}
                            <img
                                        ...
                                        <div className="flex flex-col gap-4 md:col-span-2 justify-center">
                                            ...
                                            <div ref={formCategoryRef}>
                                                ...
                                                <div className="relative group/select">
                                                    <input
                                                        type="text"
                                                        className="w-full pl-10 pr-4 py-3 rounded-xl bg-gray-50 border border-transparent text-sm focus:bg-white focus:ring-2 focus:ring-[#526f52]/40 focus:border-transparent text-[#2d3748] transition-all cursor-text shadow-[inset_0_1px_2px_rgba(0,0,0,0.05)]"
                                                        value={formData.category}
                                                        onFocus={() => setShowFormCategoryMenu(true)}
                                                        onChange={(e) => {
                                                            setFormData({ ...formData, category: e.target.value });
                                                            setShowFormCategoryMenu(true);
                                                        }}
                                                        placeholder="בחר או הקלד חדש..."
                                                    />
                                                    <button
                                                        type="button"
                                                        onClick={() => setShowFormCategoryMenu(!showFormCategoryMenu)}
                                                        className="absolute inset-y-0 left-0 w-12 flex items-center justify-center text-gray-400 hover:text-[#526f52] transition-colors border-r border-transparent hover:bg-gray-100/50 rounded-l-xl"
                                                    >
                                                        <span className="material-symbols-outlined text-[24px]">
                                                            {showFormCategoryMenu ? 'arrow_drop_up' : 'arrow_drop_down'}
                                                        </span>
                                                    </button>
                                                    {showFormCategoryMenu && (
                                                        <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-[16px] shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100 py-2 z-50 max-h-48 overflow-y-auto custom-scrollbar">
                                                            {allCategories.filter(c => c.id !== 'all' && c.label.includes(formData.category)).length > 0 ? (
                                                                allCategories.filter(c => c.id !== 'all' && c.label.includes(formData.category)).map((c) => (
                                                                    <button
                                                                        key={c.id}
                                                                        type="button"
                                                                        onClick={() => {
                                                                        ...
                                                                        className="w-full text-right px-4 py-3 text-sm text-[#2d3748] hover:bg-[#f0fdf4] hover:text-[#166534] hover:font-bold transition-colors"
                                                                    >
                                                                        {c.label}
                                                                    </button>
                                                                ))
                                                            ) : (
                                                                <div className="px-4 py-3 text-sm text-gray-500 text-right font-medium bg-gray-50/50">
                                                                    {formData.category ? `תיצור קטגוריה חדשה בשם "${formData.category}"` : 'אין קטגוריות מתאימות לחיפוש...'}
                                                                </div>
                                                            )}
                                                        </div>
                                                    )}

...
function Wallets() {
    ...
    const [formData, setFormData] = useState({
        name: '', walletNumber: '', description: '', initialBalance: '', currentBalance: '', notes: '', userIds: []
    });
    ...
    }, [fetchWallets, fetchUsers, currentUser?.role]);
    ...
    const handleOpenEdit = (e, wallet) => {
        ...
        setFormData({
            ...
            initialBalance: '', // Not used on edit
                        ...
                        <div className="p-8 overflow-y-auto custom-scrollbar">
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {!isEditMode && (
                                        <div>
                                            <label className="block text-sm font-bold text-gray-700 mb-1">יתרה התחלתית *</label>
                                            <input type="number" required={!isEditMode} className="w-full px-4 py-3 rounded-xl bg-gray-50 border-none text-sm focus:ring-2 focus:ring-[#526f52]/20" value={formData.initialBalance} onChange={(e) => setFormData({ ...formData, initialBalance: e.target.value })} />
                                        </div>
                                    )}
                                </div>
                                ...
                                {isEditMode && (
                                    <div className="space-y-4">
                                        ...
                                        {parseFloat(formData.currentBalance) !== currentWallet?.currentBalance && (
                                            <div className="animate-fade-in-up">
                                                <label className="block text-sm font-bold text-[#2d3748] mb-1">סיבת העדכון (יופיע בהיסטוריה)</label>
                                                <input type="text" placeholder="לדוגמה: זיכוי שגוי, הפקדת מזומן..." className="w-full px-4 py-3 rounded-xl bg-yellow-50/50 border border-yellow-100 text-sm focus:ring-2 focus:ring-yellow-500/20" value={formData.notes || ''} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} required />
                                            </div>
                                        )}
                                    </div>
                                )}

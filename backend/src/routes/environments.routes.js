...
// PUT /api/environments/:id/display-name - update environment name (admin/superadmin)
router.put('/:id/display-name', authorizeRoles('admin', 'superadmin'), async (req, res, next) => {
    try {
        const { id } = req.params;
        const nextName = String(req.body?.name || '').trim();
        const incomingSymbolImageUrls = req.body?.symbolImageUrls;
        ...
        if (countEmojiSymbols(nextName) > 4) {
            return res.status(400).json({ error: 'Environment name can include up to 4 symbols' });
        }

        if (req.user.role === 'admin' && req.user.environmentId !== id) {
            return res.status(403).json({ error: 'Admins can only rename their active environment' });
        }

        let normalizedSymbolImageUrls;
        if (incomingSymbolImageUrls !== undefined) {
            if (!Array.isArray(incomingSymbolImageUrls)) {
                return res.status(400).json({ error: 'symbolImageUrls must be an array' });
            }

            normalizedSymbolImageUrls = incomingSymbolImageUrls
        ...
        if (normalizedSymbolImageUrls !== undefined) {
            updateData.symbolImageUrls = normalizedSymbolImageUrls;
        }
...
// GET /api/environments - list environments with stats
router.get('/', authorizeRoles('admin', 'superadmin'), async (req, res, next) => {
    try {
        const where = req.user.role === 'admin'
            ? { isActive: true, id: req.user.environmentId }
            : { isActive: true };
...
router.post('/superadmins', async (req, res, next) => {
    try {
        ...
        const normalizedPersonalNumber = typeof personalNumber === 'string' ? personalNumber.trim() : '';
        if (!normalizedPersonalNumber) {
            return res.status(400).json({ error: 'personalNumber is required' });
        }

        const existingByPersonalNumber = await usersPrisma.user.findUnique({
            where: { personalNumber: normalizedPersonalNumber }
        });

        if (existingByPersonalNumber) {
            const updatedUser = await usersPrisma.user.update({
                where: { id: existingByPersonalNumber.id },
                data: {
                    role: 'superadmin',
                    isActive: true
                }
            });
            const { passwordHash: _ignoredUpdatedHash, ...safeUpdatedUser } = updatedUser;
            return res.json(safeUpdatedUser);
        }

        if (!username || !password || !fullName) {
            return res.status(400).json({ error: 'username, password, fullName are required for new user' });
        }
        ...
        if (error.code === 'P2002') return res.status(400).json({ error: 'User with this username or personal number already exists' });
        return next(error);
    }
});
